...
__forceinline__ __device__ void write_integral(double *output,
                                               const double x_pairs[],
                                               const double y_pairs[],
                                               const double z_pairs[],
          ...
          static_for<0, ncart(j_angular)>([&]<int j>() {
            ...
            if constexpr (abs_cj > 1e-15) {
              ...
              expression += cij * x_pairs[xi * stride + xj] *
                            y_pairs[yi * stride + yj] *
                            z_pairs[zi * stride + zj];
            }
          });
        }
      });
